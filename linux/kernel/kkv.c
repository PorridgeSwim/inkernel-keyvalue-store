...
long (*kkv_put_ptr)(uint32_t key, void __user *val, size_t size, int flags) = NULL;
EXPORT_SYMBOL(kkv_put_ptr);

long (*kkv_get_ptr)(uint32_t key, void __user *val, size_t size, int flags) = NULL;
...
SYSCALL_DEFINE4(kkv_put, uint32_t, key, void __user *, val, size_t, size, int, flags)
{
	if (kkv_put_ptr)
...
SYSCALL_DEFINE4(kkv_get, uint32_t, key, void __user *, val, size_t, size, int, flags)
{
	if (kkv_get_ptr)
